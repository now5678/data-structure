/*************************************************************************
	> File Name: HZOJ-838.cpp
	> Author: 
	> Mail: 
	> Created Time: Mon 15 Jul 2024 08:04:02 AM CST
 ************************************************************************/
/*************************************************************************
    > File Name: solve.cpp
    > Author:
...
int func(queue<int> que1, queue<int> que2, queue<int> que3) {
    ...
    while(!que1.empty() && !que2.empty() && !que3.empty())
    {
        ...
        int tmp_num = min_num(a, b, c);
        if (a == tmp_num) que1.pop();
        if (b == tmp_num) que2.pop();
        if (c == tmp_num) que3.pop();
    }
    return ans;
}
